...
const premierBtnRetour = formulaire.querySelector(".navigation-section>div:first-of-type");
...
const resumeTransporteur = formulaire.querySelector("[data-name='transporteur']");
...
function formatterChamps(declencheur){
    ...
    if(declencheur.name !== "optionsSuccursales" && declencheur.name !== "commentaire" ){
        if (declencheur.type == "email"){
           ...
           declencheur.value = declencheur.value.replace(/(\d{3})([\-\s]?)(\d{3})([\-\s]?)(\d{4})/, "$1-$3-$5");        
        }else{
...
function gererChampException(declencheur){
    ...
    const resumeLivraisonEnMagasin = formulaire.querySelector("[data-name='livraisonMagasin']");
    const resumeSuccursale = formulaire.querySelector("[data-name='optionsSuccursales']");
...
/**
 * Fonction pour afficher la date d'aujourd'hui dans le calendrier pour la date de livraison idéale
 * @returns la date d'aujourd'hui
 */
...
function validerChamp(declencheur){
    ...
    if (estValide) {
        ...
        if (sectionInvalide == true) {
            sectionParent.querySelector(".bouton[data-direction='1']").classList.add("inactif");   
        } else {
            sectionParent.querySelector(".bouton[data-direction='1']").classList.remove("inactif");
        }     
    }  
}
